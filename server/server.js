...
async function handleDirections(request, response, url) {
  if (request.method !== 'GET') {
    return sendJson(response, 405, { message: 'GET 요청만 지원합니다.' }, { Allow: 'GET' })
  }
...
const server = http.createServer((request, response) => {
  ...
  if (url.pathname === '/api/directions') {
    return handleDirections(request, response, url)
  }

  return sendJson(response, 404, { message: '요청한 경로를 찾을 수 없습니다.' })
})
