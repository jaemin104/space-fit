...
function HomePage({ onNavigate }: { onNavigate: (tab: TabId) => void }) {
  ...
  return <div className="screen home-screen">
    ...
    <section className="summary-section">
      <div className="summary-head"><span>오늘 요약 · 완료 1건</span></div>
      <div className="net-earning">{net.toLocaleString()}<small>원</small></div>
    </section>
    <section className="recent-section"><h2>최근 운송 내역</h2>{trips.map((trip) => <article key={trip.id}><div><strong>{trip.from} → {trip.to}</strong><span>{trip.date} · {trip.category} 운송</span></div><div><strong>{trip.price.toLocaleString()}원</strong><span className="complete-badge">완료</span></div></article>)}</section>
  </div>
}
